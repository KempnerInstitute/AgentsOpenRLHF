#!/bin/bash
#SBATCH --job-name="m8base"
#SBATCH --account=kempner_undergrads
#SBATCH --output /n/holylfs06/LABS/kempner_undergrads/Lab/ellenma/openrlhf-proj/AgentsOpenRLHF/rl_run_logs/8m_%A.log
#SBATCH --error /n/holylfs06/LABS/kempner_undergrads/Lab/ellenma/openrlhf-proj/AgentsOpenRLHF/rl_run_logs/8m_error_%j.out
#SBATCH --export=ALL
#SBATCH --nodes=2
...
srun --overlap -N 1 -n 1 -w "$head_node" ray job submit --address="http://127.0.0.1:8265" \
  ...
  --save_path /n/holylfs06/LABS/kempner_undergrads/Lab/ellenma/openrlhf-proj/AgentsOpenRLHF/openrlhf_artifacts/frozenlake_rl_multiturn/rl_qwen8 \
  --vllm_num_engines 4 \
  ...
  --use_wandb True \
  --wandb_project frozenlake_rl_multiturn \
  --wandb_run_name 8B_base \
  --agent_func_path /n/holylfs06/LABS/kempner_undergrads/Lab/ellenma/openrlhf-proj/AgentsOpenRLHF/scripts/fl_agent_multiturn.py \
  --prompt_data /n/holylfs06/LABS/kempner_undergrads/Lab/ellenma/openrlhf-proj/AgentsOpenRLHF/data/frozen_lake/rl_train.jsonl \
  --ckpt_path /n/netscratch/kempner_undergrads/Lab/ellenma/openrlhf-rl-flake-checkpoints-multiturn/qw8 \
  --input_key prompt \
  --async_train \
  --save_steps 100 \
