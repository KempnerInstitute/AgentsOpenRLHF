...
class FrozenLakeAgentInstanceTool():
    ...
    async def step(self, observation, response, label, **kwargs) -> Dict[str, Any]:
        ...
        if tool_use:
            ...
            if reward==1 and not self.sim_reward:
                self.sim_reward=True 
                
            elif reward ==1 and self.sim_reward:
                reward = 0
            
            if self.n_sim == 1000:
                return {
                    "rewards": np.array(reward),        # intermediate reward 
            ...
            if done:
                return {
                    ...
                    "scores": np.array([reward]),       # true completion marking
            ...
            next_prompt = fl.make_prompt_no_tool_sim(self.env_str) 
